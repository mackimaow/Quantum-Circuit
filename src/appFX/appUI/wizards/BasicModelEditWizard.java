...
import appFX.appUI.utils.ListSelectionPaneWrapper;
...
public class BasicModelEditWizard extends AppFXMLComponent implements Initializable, ChangeListener<BasicModelType> {
	...
	public static void createNewGate(String name) {
		...
		dialog.setScene(new Scene((Parent) new BasicModelEditWizard(dialog, name).loadAsNode(), 650, 700));
	...
	private static void openGateEditableView(String name, boolean editAsNewModel) { 
		...
		Stage dialog = new Stage();
		dialog.setScene(new Scene((Parent) new BasicModelEditWizard(dialog, gm, editAsNewModel).loadAsNode(), 650, 700));
		dialog.initOwner(primary);
		dialog.initModality(Modality.APPLICATION_MODAL);
	...
	private BasicModelEditWizard (Stage stage, String name) {
	...
	private BasicModelEditWizard (Stage stage, boolean editAsNewModel) {
	...
	private BasicModelEditWizard(Stage stage, BasicModel gm, boolean editAsNewModel) {
