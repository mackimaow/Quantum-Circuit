...
public class ControlToolAction extends ToolAction {
	...
	public void buttonPressed(double column, double offGridX, double row, double offGridY) {
		...
		SolderedPin sp = cbv.getCircuitBoardModel().getSolderPinAt(rowInt, columnInt);
		...
		if(currSG == null) {
			...
			Pair<Integer, Integer> rowBounds = cbv.getCircuitBoardModel().getSolderedGateBodyBounds(rowInt, columnInt);
			rowSelStart = rowBounds.first();
			rowSelEnd = rowBounds.second();
			colSel = columnInt;
			...
			cbv.getCircuitBoardModel().placeControl(rowInt, rowSelStart, columnInt, controlType);
