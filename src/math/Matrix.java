...
import math.operators.OperatorSet;
...
public class Matrix<T> implements Serializable {
	...
	public static <T> Matrix<T> identity(T elementToInferOperator, int size){
		OperatorSet<T> operation = OperatorSet.inferOperatorSet(elementToInferOperator);
	...
	protected Matrix(T[] comps, int rows, int columns, OperatorSet<T> o) {
		this.comps = comps;
		this.rows = rows;
	...
	public Matrix(int rows, int columns, T ... components){
		o = OperatorSet.inferOperatorSet(components[0]);
	...
	public Matrix(T elementToInferOperator, int rows, int columns){
		o = OperatorSet.inferOperatorSet(elementToInferOperator);
	...
	public T trace() {
        ...
        for (int x = 0; x < getColumns(); ++x) {
            for (int y = 0; y < getRows(); ++y) {
                temp = o.add(temp, comps[y + x * getRows()]);
            }
        }
        return temp;
    }
