...
public class MathInteger implements MathValue {
	private static final long serialVersionUID = 282098781957287735L;
	
	long number;
	
	public MathInteger(long number) {
		this.number = number;
	}
	...
	public int compare(MathInteger value) {
		return Long.compare(number, value.number);
	}
	...
	public MathValue add(MathInteger value) {
		return new MathInteger(number + value.getNumber());
	}
	
	public MathValue add(MathReal value) {
		return MathReal.toMathReal(number + value.getNumber());
	}
	...
	public MathValue xOr(MathInteger value) {
		return new MathInteger(number ^ value.number);
	}
	...
	@Override
	public MathValue index(MathInteger value) {
		int offset = ZERO_BASED? 0 : 1;
		long index = value.number;
		if(index - offset < 0)
			throw new InvalidOperation("Cannot index number at a negative " + (ZERO_BASED? "number":"or a zero number"));
		long target = number;
		return new MathBoolean(((target >>> index - offset) & 1) == 1);
	}
